...
class DetBenchTrain(nn.Module):
    ...
    def forward(self, x, target):
        ...
        class_is_nan = np.any([np.any(np.isnan(co.detach().cpu().numpy())) for co in class_out])
        box_is_nan = np.any([np.any(np.isnan(co.detach().cpu().numpy())) for co in box_out])
        if box_is_nan or class_is_nan:
            print(loss, class_loss, box_loss)
